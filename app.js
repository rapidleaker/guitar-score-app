(() => {
  ...
  function getScrollState() {
    return {
      ...
      scoreLeft: els.score ? els.score.scrollLeft : 0,
      scoreTop: els.score ? els.score.scrollTop : 0
    };
  }
  ...
  function getCharX(row, index) {
    ...
    if (index >= chars.length) {
      return (
        chars[chars.length - 1].getBoundingClientRect().right -
        rowRect.left
      );
    }
  ...
  function renderFontSettings() {
    ...
    const lyricSize = clampFontSize(
      song.lyricFontSize,
      DEFAULT_LYRIC_FONT_SIZE
    );

    const chordSize = clampFontSize(
      song.chordFontSize,
      DEFAULT_CHORD_FONT_SIZE
    );
    ...
    document.documentElement.style.setProperty(
      '--lyric-font-family',
      fontCss(song.lyricFontFamily)
    );

    document.documentElement.style.setProperty(
      '--chord-font-family',
      fontCss(song.chordFontFamily)
    );
  }

  function fontCss(value) {
    return value.includes(' ') ||
      /[ぁ-んァ-ン一-龯]/.test(value)
      ? `"${value}"`
      : value;
  }
  ...
  function duplicateSong(id) {
    ...
    copied.chords.forEach(chord => {
      chord.id = uid('chord');
      chord.lineId =
        idMap.get(chord.lineId) || chord.lineId;
    });
  ...
  els.modes.lyrics.onclick = () => {
    switchMode('lyrics');
  };

  els.modes.chords.onclick = () => {
    switchMode('chords');
  };

  els.modes.preview.onclick = () => {
    switchMode('preview');
  };
...
document.addEventListener(
  ...
  event => {
    ...
    const clickedInlineInput =
      event.target.closest('.inline-chord-input');
